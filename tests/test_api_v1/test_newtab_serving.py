...
class TestNewtabServing(BaseTestCase):
    ...
    def test_cookie_reset(self):
        ...
        session = signer.loads(
            orig_cookie[self.app.config['SESSION_COOKIE_NAME']])
        original_data = json.loads(decrypt(session['ciphertext'],
                                   session['iv']))

        expired_data = deepcopy(original_data)

        expired_data['created'] = calendar.timegm(
            (datetime.utcnow() - timedelta(
                days=self.app.config['SESSION_MAX_AGE'])).timetuple())

        ciphertext, iv = encrypt(json.dumps(expired_data))
        expired_signed_data = signer.dumps({
            'ciphertext': ciphertext,
            'iv': iv})
        self.client.set_cookie('localhost.local',
                               key=self.app.config['SESSION_COOKIE_NAME'],
                               value=expired_signed_data)
        ...
        new_data = None
        with self.client.session_transaction() as session:
            new_data = json.loads(decrypt(session['ciphertext'],
                                  session['iv']))
